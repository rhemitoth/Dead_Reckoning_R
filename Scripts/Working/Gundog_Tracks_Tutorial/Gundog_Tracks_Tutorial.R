...
df <- read.delim("Data/Gundog_Tutorial/Test.Data.P10A_selection_split#1_0.txt") #Change as appropriate
source("Scripts/Working/Gundog_Tracks_Tutorial/Gundog.Tracks.R") #Change as appropriate
...
penguin.track <- Gundog.Tracks(TS = df.sub$timestamp, 
                               h = df.sub$Yaw, 
                               v = df.sub$VeDBA.sm, 
                               elv = 0, 
                               p = NULL, 
                               ...
                               m = 1.5, 
                               c = 0, 
                               ME = df.sub$Marked.event, 
                               lo = -63.86774, 
                               la = -42.08654, 
                               VP.lon = df.sub$GPS.Longitude, 
                               VP.lat = df.sub$GPS.Latitude, 
                               VP.ME = TRUE, 
                               method = "distance", 
                               thresh.d = 12, 
                               dist.step = 5, 
